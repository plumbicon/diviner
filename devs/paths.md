...
Скрипты, которые **не являются частью проекта**, в репозиторий не кладутся — их место в `/tmp`:

| Тип | Где | Примеры |
|---|---|---|
| Автоматизация **над** проектом | `/tmp` | дёрнуть API счетов, статус ботов, диагностика, ops |
| Автогенерация отчётов | `/tmp` | P&L-сводки, статистика, дашборды |
| Разовый анализ | `/tmp` | помесячная доходность, ad-hoc выборки, проверки гипотез |
| Жизненный цикл стратегий | в проекте | `src/strategies/*/scripts/` (обучение/тюнинг), `src/strategies/scripts/` |
| Раннер бэктеста | в проекте | `scripts/backtest.mjs` |
| Сам движок | в проекте | `src/core/`, `src/broker/`, CLI |

Правило: утилита **работает над** проектом или **анализирует** его → `/tmp`; утилита
**производит/обучает** стратегии или **есть** движок → в проект.
