#!/usr/bin/env bash
# Merge official Tinkoff 1d candles (Dec 2024 – Dec 2025) into all *_2026_1m.parquet files.
#
...
DATA="data"
FROM="2025-12-01"    # enough for PREVIOUS_CLOSE_LOOKBACK_DAYS=14 before Jan 2025
TO="2026-06-08"
DELAY=150            # ms between API requests
WORKERS=4
STAGGER=4            # seconds between worker starts

TICKERS=($(ls "$DATA"/*_2026_1m.parquet 2>/dev/null \
  | xargs -I{} basename {} | sed 's/_2026_1m.parquet//' | sort))

echo "Tickers: ${#TICKERS[@]}"
echo "Adding 1d candles from $FROM to $TO (2026) …"
